...
plot.list <- list()
    ...
    # set colors for taxonomic groups in community plots to avoid confusion with taxonomic classes
        group.colors <- c("mammals/birds" = "#00BFC4", "amphibians/reptiles" = "#F8766D")
...
# LSU individual
    ...
    plot.list$LSU.individual.elevation <- LSU.individual.predictions$elev %>%
        ...
        geom_line() + labs(x = "elevation (m)", y = "occupancy", color = "taxonomic class") + theme(legend.title = element_blank())
    ...
    plot.list$LSU.individual.reserve <- LSU.individual.predictions$reserve %>%
        ...
        geom_line() + labs(x = "distance to reserve edge (m)", y = "occupancy", color = "taxonomic class") + theme(legend.title = element_blank())
    ...
    plot.list$LSU.individual.detection <- LSU.individual.predictions$numleeches %>%
        ...
        geom_line() + labs(x = "number of leeches", y = "detection", color = "taxonomic class") + theme(legend.title = element_blank())
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_Fig4_FigS3cdef_covariates.sessioninfo.txt"))
