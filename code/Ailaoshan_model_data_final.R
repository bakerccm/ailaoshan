# R code to prepare to run final occupancy models
...
# parameters to monitor
    ...
    params <- c("estimated.occupancy", "Nsite", "z", "beta0", "beta", "gamma0", "mu.eta", "mu.beta", "rho")
...
# save data to files for modelling
    ...
    for (d in datasets) {
        ...
        jags.data.filename <- here("rdata", paste0("Ailaoshan_model_data_final_", d, ".rdata"))
...
# save prepared MCMC data
    ...
    save(inits, leech, leech.supplement, leech.augmented, mcmc.settings, model.data, numleeches,
         params, site.covariates.scaled, y, z.start, file = here("rdata", "Ailaoshan_model_data_final.rdata"))

# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_model_data_final.sessioninfo.txt"))
