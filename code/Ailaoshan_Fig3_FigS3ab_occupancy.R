    ...
    # plot Fig S3a: LSU richness
        OTU.richness %>% filter(dataset == "LSU") %>%
            ggplot(aes(x = richness, color = source)) +
                geom_histogram(fill = "white", alpha = 0.5, position = "identity", binwidth = 2) + coord_cartesian(xlim=c(0,40), ylim=c(0,55)) +
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_Fig3_FigS3ab_occupancy.sessioninfo.txt"))
