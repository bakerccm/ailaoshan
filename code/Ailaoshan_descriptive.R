...
# species
    ...
    # how many species were detected in each Polygon_ID?
        ...
        richness.per.PolygonID <- leech %>%
            ...
            summarize(present = ifelse(sum(reads)>0, 1, 0), .groups = "drop_last") %>%
            summarize(richness = sum(present), .groups = "drop")
    ...
    # how many Lab_IDs or Polygon_IDs was each species detected in?
        ...
        leech %>%
            group_by(dataset, Polygon_ID, OTU) %>% summarize(present = ifelse(sum(reads)>0, 1, 0)) %>%
            group_by(dataset, OTU) %>% summarize(num.PolygonIDs = sum(present)) %>%
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_descriptive.sessioninfo.txt"))
