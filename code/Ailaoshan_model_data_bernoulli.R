# R code to prepare to run occupancy models with Bayesian variable selection
...
# parameters to monitor
    ...
    params <- c("delta.beta", "mu.beta", "sigma.beta", "mu.eta", "sigma.eta")
...
# save data to files for modelling
    ...
    for (d in datasets) {
        ...
        jags.data.filename <- here("rdata", paste0("Ailaoshan_model_data_bernoulli_", d, ".rdata"))
...
# save prepared MCMC data
    ...
    save(inits, leech, leech.supplement, leech.augmented, mcmc.settings, model.data, numleeches,
         params, site.covariates.scaled, y, z.start, file = here("rdata", "Ailaoshan_model_data_bernoulli.rdata"))

# save session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_model_data_bernoulli.sessioninfo.txt"))
