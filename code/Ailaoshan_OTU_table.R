...
#  -- preOTUs with the same final taxonomy assigned manually by DY are collapsed to give final OTUs
...
# ubiquity of human reads
    ...
    # fraction of PolygonIDs with human reads
        leech %>%
            ...
            summarize(humans.present = ifelse(sum(reads) > 0, 1, 0), .groups="drop_last") %>%
            summarize(`Polygon_IDs with human reads` = sum(humans.present) / n(), .groups="drop")
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_OTU_table.sessioninfo.txt"))
