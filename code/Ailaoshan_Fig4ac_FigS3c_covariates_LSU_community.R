...
# elevation
    ...
    elev.pred <- array(NA, dim = c(nrow(elev.vals), nsamp, 2)) # empty array to be filled
    for(s in 1:nsamp){
        elev.pred[,s,1] <- plogis(model.output$BUGSoutput$sims.list$mu.eta[s,1,1] + model.output$BUGSoutput$sims.list$mu.beta[s,1] * elev.vals$scaled)        # psi ~ elev for group 1 (i.e. mammals/birds)
        elev.pred[,s,2] <- plogis(model.output$BUGSoutput$sims.list$mu.eta[s,1,2] + model.output$BUGSoutput$sims.list$mu.beta[s,1] * elev.vals$scaled)        # psi ~ elev for group 2 (i.e. amphibians/reptiles)
    }
...
# reserve
    ...
    # generate community occupancy predictions based on the mcmc iterations
    reserve.pred <- array(NA, dim = c(nrow(reserve.vals), nsamp, 2)) # empty array to be filled
    for(s in 1:nsamp){
        reserve.pred[,s,1] <- plogis(model.output$BUGSoutput$sims.list$mu.eta[s,1,1] + model.output$BUGSoutput$sims.list$mu.beta[s,2] * reserve.vals$scaled)     # psi ~ reserve for group 1 (i.e. mammals/birds)
        reserve.pred[,s,2] <- plogis(model.output$BUGSoutput$sims.list$mu.eta[s,1,2] + model.output$BUGSoutput$sims.list$mu.beta[s,2] * reserve.vals$scaled)     # psi ~ reserve for group 2 (i.e. amphibians/reptiles)
    }
...
# detection estimates
    ...
    detection.pred <- array(NA, dim = c(length(numleeches.vals), nsamp, 2)) # empty array to be filled
    for(s in 1:nsamp){
        detection.pred[,s,1] <- 1 - ((1 - plogis(model.output$BUGSoutput$sims.list$mu.eta[s,2,1])) ^ (numleeches.vals/100))     # p ~ numleeches for group 1 (i.e. mammals/birds)
        detection.pred[,s,2] <- 1 - ((1 - plogis(model.output$BUGSoutput$sims.list$mu.eta[s,2,2])) ^ (numleeches.vals/100))     # p ~ numleeches for group 2 (i.e. amphibians/reptiles)
    }
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_Fig4ac_FigS3c_covariates_LSU_community.sessioninfo.txt"))
