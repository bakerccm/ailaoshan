...
# LSU elevation
    ...
    LSU.plot.elev <- beta.output$LSU$elev %>%
        left_join(taxa, by = "OTU") %>% mutate(class = tolower(class)) %>%
        ...
        # add dummy reptile rows to ensure reptiles facet is wide enough
            bind_rows(tibble(consensus.short = paste0("dummy", 1:4), class = rep("reptiles",4), mean = c(-2,-2,2,2))) %>%
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_FigS5_FigS6_FigS7_occupancy_slopes.sessioninfo.txt"))
