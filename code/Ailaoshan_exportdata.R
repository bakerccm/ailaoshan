...
#     add '=' to tied ranks, check capitalisation on common names
    ...
    taxa.iucn.estimates %>%
        ...
        write.table(here("tables","Table2_top20_OTUs_latex.txt"), quote = FALSE, sep = " & ", row.names = FALSE, eol = " \\\\\n")
...
# write taxon data to xlsx file
    ...
    write_xlsx(xlsx.export.taxa,
        path = here("tables","leeches_supplement_file_S2.xlsx"),
...
# export OTU tables and metadata as xlsx files
    ...
    write_xlsx(xlsx.export.OTUtables,
        path = here("tables","leeches_supplement_file_S5.xlsx"),
        col_names = TRUE, format_headers = TRUE)
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_exportdata.sessioninfo.txt"))
