...
# get IUCN API key from command line arguments
...
# get taxon lists
    ...
    taxa <- leech %>%
        ...
        # calculate fraction of sites occupied
            summarize(`observed occupancy` = sum(occupied) / n(), .groups = "drop") %>%
...
# session info
writeLines(capture.output(sessionInfo()), here("sessioninfo", "Ailaoshan_IUCNdata.sessioninfo.txt"))
